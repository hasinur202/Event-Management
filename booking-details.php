...
<h3 style="padding: 5px; text-align: center; background: orange; color: white; border: 1px solid #ddd; border-radius: 5px;" class="aside-title uppercase"><a href="event-details.php?evntid=<?php echo htmlentities($row->evtid);?>"><?php echo htmlentities($row->EventName);?></a> Booking Details</h3>
<div class="row">
<div class="col-md-12 col-sm-6 col-xs-12 lyt-left">
<div class="input-box leave-ib">
<div class="table-responsive">

<table border="1" class="table table-hover" style="background: #acdcdc; width: 100%;">
        ...
        <th>Booking Status</th>
            ...
            if($bstatus==""){ ?>
            <button class="btn btn-success btn-sm"><?php echo htmlentities("Not confirmed Yet");?></button>

            <?php } else { ?>
                <button class="btn btn-success btn-sm"><?php echo htmlentities($bstatus);?></button>
        ...
        <tr>
            ...
            <td><?php echo htmlentities($row->UserCancelRemark);?></td>   
        </tr>
        <?php } ?>


        <tr style="color: black; background: yellow; ">

        <?php
                $uid=$_SESSION['usrid'];
                $bkngid=intval($_GET['bkid']);
                $sqlpay = "SELECT * FROM transaction WHERE id=(SELECT max(id) FROM transaction);";

                $querypay = $dbh -> prepare($sqlpay);
                $querypay->execute();
                $resultspay=$querypay->fetchAll(PDO::FETCH_OBJ);
                if($querypay->rowCount() > 0)
                {
                foreach($resultspay as $rowpay)
                { 

         ?>

            <th>Transaction ID</th>    
            <td><?php echo htmlentities($rowpay->tran_id);?></td>
            <th>Payment Type</th>    
            <td><?php echo htmlentities($rowpay->card_type);?></td>
            <th>Amount</th>    
            <td><?php echo htmlentities($rowpay->amount);?></td>
        ...
        -->
            ...
            <button class="btn btn-info"><i class="fa fa-print fa-2x" aria-hidden="true" OnClick="CallPrint(this.value)"></i></button>
            </td>
